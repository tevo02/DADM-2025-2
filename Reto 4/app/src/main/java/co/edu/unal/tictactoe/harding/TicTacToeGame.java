...
public class TicTacToeGame {
    ...
    private char mBoard[];
    public static final int BOARD_SIZE = 9;  // <- público
    ...
    private int getRandomMove() {
        ...
        if (!available.isEmpty()) {
            return available.get(mRand.nextInt(available.size()));
        }
        return -1;
    }
    ...
    // 0=no hay ganador, 1=empate, 2=humano, 3=CPU
    public int checkForWinner() {
        ...
        for (int i = 0; i < BOARD_SIZE; i++) if (mBoard[i] == OPEN_SPOT) return 0;
        return 1;
    }
    ...
    public char[] getBoardState() {
        char[] out = new char[9];
        for (int i = 0; i < 9; i++) {
            out[i] = getBoardOccupant(i); // devuelve 'X', 'O' o ' ' (vacío)
        }
        return out;
    }
    ...
    public void setBoardState(char[] state) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (c == HUMAN_PLAYER || c == COMPUTER_PLAYER) {
                // setMove() debe permitir reponer sin validar turno (si no, añade un setter directo)
                setMove(c, i);
            }
        }
    }
